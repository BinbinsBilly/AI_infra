...
- **矩阵维度优先 (用户重点要求)**: 涉及张量运算时, 必须显式写出每个矩阵的 shape 及其变化链 (如 `x(d_model) @ W_q(d_model, n_heads·d_head) → Q(n_heads, d_head)`); 严格区分"个数"与"维度"两类易混量 (如 n_heads 是头的个数, d_head 是每个头的维度, 两者独立); 讲 shape 变化尽量配示意图 (矩阵长方块图/数据流图), 用具体数字 (如 LLaMA-7B: 4096/32/128) 举例
- **公式先行 (用户重点要求)**: 引入任何激活函数/归一化/算子时, 先给出数学公式 (LaTeX 或纯文本), 再解释直觉与对比; 对比类内容尽量以表格形式呈现, 列出公式、定义域/值域、关键性质; 公式与直觉配图互补, 不互相替代
- **缩写必带全称 (用户重点要求)**: 任何缩写词 (如 EM、DP、OOV、KV、GQA 等) 首次出现时必须给出英文全称 + 中文释义; 后续可直接用缩写; 遇到框架/工具缩写 (如 GEMM、TP、PP、EP) 同样处理
